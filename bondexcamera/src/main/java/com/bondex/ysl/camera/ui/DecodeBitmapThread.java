...
public class DecodeBitmapThread extends Thread {
    ...
    /*回调*/
    private DecodeBitmapCallback callback;




    public DecodeBitmapThread(Bitmap scanBitmap,  DecodeBitmapCallback callback) {
    ...
    public void run() {
        ...
        if (scanBitmap == null || callback == null) {
            return;
        }
...
//        如果解析失败，就在解析三次
        ...
        try {
            rawResult = multiFormatReader.decodeWithState(new BinaryBitmap(new GlobalHistogramBinarizer(new BitmapLuminanceSource(scanBitmap))));
            Log.i("bitmap 解析结果", rawResult.getText());

        } catch (Exception e) {
            e.printStackTrace();
            Log.i("bitmap 解析的图片结果", "失败");
        }
